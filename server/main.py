...
def embedding_map(method: str = "tsne"):
    ...
    This used to also k-means the library and name each group from
    `labels.py`'s vocabulary, drawn as a legend over the cloud. It was removed:
    the groups were real but the *names* were not useful, for the reason
    `zero_shot_logp` now exists to fix — a bare nearest-label lookup answers
    with something for every centroid, and the assignment was forced to be
    distinct, so a library with two kinds of landscape got one named after its
    runner-up. What the vocabulary is still for is search, which is why
    `label_vectors.npz` and the whole of `labels.py` remain.
    """
